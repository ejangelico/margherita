...
rows = cur.fetchall()
if len(rows) > 1: raise Exception('pvr found multiple alias correspondences')
if len(rows) == 1: pvname = rows[0][0]

# find PV in database
cur.execute("SELECT * FROM pvs WHERE pvname = '"+pvname+"'")
rows = cur.fetchall()
if len(rows) > 1: raise Exception('pvr found multiple entries for PV '+pvname)
if len(rows) == 0: raise Exception("pvr did not find a PV named "+pvname+" or an alias "+pvname+" in context "+calledfrom)
...
except: print "Can\'t find level-0 path in table admin"
rows = cur.fetchall()
...
#try: cur.execute("UPDATE pvs SET value = '"+res+"' WHERE pvname = '"+pvname+"'")
#except: raise Exception("Can\'t find PV name "+pvname+" in pvs database")
