...
class RGA(object): # Instantiate one of these as an interface to the real RGA
	...
	def Degas(self, timeparam=None):
		if(timeparam is None):
			#this is default option, degasses for 3 mins
			self.port.write('DG*\r')
		else:
			if(timeparam >= 0 and timeparam <= 20):
				self.port.write('DGparam'+str(timeparam)+'\r')
			else:
				print "Degas parameter must be integer from 0 to 20 (minutes of degassing)"
				return
		...
		while(self.port.inWaiting() == 0):
			time.sleep(10)
			tf = time.time()
			print "Degassing has elapsed " + str((tf - ti)/60.0) + " minutes"
	...
	def AScanStart(self):# this function performs an analog scan
                print "Writing params and setting variables"
                #need two Write Params because, and I don't know why, 
                #rga does not accept a start_mz if it is greater than the previous
                #start_mz. You can send it directly with port.write('mi\r') and it 
                #wont take the variable unless you do it twice
		self.WriteParams()
                self.WriteParams()
		self.CheckParams()
		...
		self.file = open('/home/pi/MargheritaCode/data/RGA/RGA-{}.txt'.format(time.strftime('%m%d%y%H%M%S')),'a')
                self.scan_complete=False

        def partial_pressure_converter(self,x):
                p = struct.unpack('<I',x)[0]*1e-16/self.p_pressure_sensitivity
                if p >  2e-3:
                        p = 2**self.scan_speed*1e-16/self.p_pressure_sensitivity
                else:
                        pass
                return p

        def total_pressure_converter(self,x):
                p = struct.unpack('<I',x)[0]*1e-16/self.t_pressure_sensitivity
                if p >  2e-3:
                        p = 2**self.scan_speed*1e-16/self.t_pressure_sensitivity
                else:
                        pass
                return p
		...
		if self.measurement < self.ap+1:
			...
			if self.measurement+points < self.ap+1:
				...
				self.file.write('\n'.join([str(self.masses[i+self.measurement])+','+str(
                                self.partial_pressure_converter(self.lastmessage[4*i:4*i+4])
        ...
        def peakScan(self, amu):
            #FC detection enable, extinguish the response
            self.port.write('hv0\r')
            time.sleep(0.3)
            #Readjust the zero for a clean baseline
            self.port.write('ca\r')
            time.sleep(0.3)
            self.port.read(100)
            #do the mass scan
            cmd = 'mr' + str(amu) + '\r'
            self.port.write(cmd)
            time.sleep(self.responsetime*3)
            self.lastmessage = self.port.read(4)
            print self.partial_pressure_converter(self.lastmessage)
            return (self.partial_pressure_converter(self.lastmessage))

        def turnoffRfDc(self):
            self.port.write('mr0\r')
            time.sleep(self.responsetime)
        
        def singleMassRead(self, mass):
            cmd = 'mr' + str(mass) + '\r'
            self.port.write(cmd)
            while self.port.inWaiting() < 4:
                time.sleep(0.1)
            self.lastmessage = self.port.read(4)
            return self.partial_pressure_converter(self.lastmessage[0:4])

        def totalPressureRead(self):
            self.port.write('tp?\r')
            while self.port.inWaiting() < 4:
                time.sleep(0.1)
            self.lastmessage = self.port.read(4)
            return self.total_pressure_converter(self.lastmessage[0:4])
