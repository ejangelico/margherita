...
class RGA(object): # Instantiate one of these as an interface to the real RGA
	...
	def Ignite(self):
        ...
        def serialRead(self):
                self.lastmessage=self.port.read(self.port.inWaiting())
                return re.split('\n\r',self.lastmessage)[0:-1]
        def serialWrite(self,messages):
                self.port.write('\r'.join(messages)+'\r')
        def cdemCheck(self):
                self.serialWrite(['hv?'])
                time.sleep(0.1)
                return int(self.serialRead()[0])
        def cdemOn(self,volts=1000):
                self.serialWrite(['hv'+str(volts),'hv?'])
                time.sleep(5)
                a=self.serialRead()
                self.errbyte=int(a[0])
                self.cdem=int(a[1])
                return abs(float(self.cdem-volts))/volts < 0.01
        def cdemOff(self):
                self.serialWrite(['hv0','hv?'])
                time.sleep(7)
                a=self.serialRead()
                self.errbyte=int(a[0])
                self.cdem=int(a[1])
                return int(self.cdem)==0
