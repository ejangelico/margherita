...
class RGA(object): # Instantiate one of these as an interface to the real RGA
	...
	def IDCheck(self,expected_id): 
		...
		time.sleep(5*self.responsetime)#this is a hack so that we read the response to the command we sent, not the previous command
