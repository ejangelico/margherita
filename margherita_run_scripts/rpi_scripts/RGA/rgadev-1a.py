...
class RGA(object): # Instantiate one of these as an interface to the real RGA
	def __init__(self, portname='/dev/ttyUSB0', baudrateset=28800, mytimeout=0.5, responsetime=0.5,
		...
		# and if they disagree, send a message to stdout
	...
	def IDCheck(self,expected_id): 
		...
		time.sleep(2*self.responsetime)#this is a hack so that we read the response to the command we sent, not the previous command
	...
	def AScanWrite(self):
		if self.measurement < self.ap+1:
			...
			if self.measurement+points < self.ap+1:
				...
				'<I',self.lastmessage[4*i:4*i+4])[0]*1e-16/self.p_pressure_sensitivity
