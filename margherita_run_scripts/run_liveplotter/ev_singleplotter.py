...
plotDate = ['171216', '171217','171218']
plotTimeStart = "02:20:00"
plotTimeEnd = "23:59:00"
...
def initializeTraces(chunk, live):
	...
	for trace in _[1:-1]:
		...
		trData = [d[int(trElements[0])] for d in windowData]
...
def assembleFigure(traces):
	...
	#fig, axes = plt.subplots(nrows = R, ncols = C, figsize=(15, 12))
	...
	for p in range(len(sortedTraces)):
		...
		axes[p].set_title(currentGroup + ";" + currentDate, fontsize=14)
		axes[p].get_yaxis().set_tick_params(labelsize=14, length=20, width=2, which='major')
		axes[p].get_xaxis().set_tick_params(labelsize=14, length=20, width=2, which='major')
	...
	for grp in sortedTraces:
		for tr in grp:
			...
			if(chnum in chan):
				...
				patches.append(Circle((x[idx], y[idx]), 70))

				#annotate circle with the temperature
				temperature = tr.getMostRecentDataPoint()
				ax.annotate(str(int(temperature)), (x[idx], y[idx]), color='k', weight='bold', fontsize=11, ha='center', va='center')
