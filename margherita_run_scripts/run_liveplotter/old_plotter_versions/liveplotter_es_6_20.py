...
def main():
		...
		tail_tempfile = 'tail_temprun0.txt'
		#label data loading
		templabels_file = 'TC_index_es.txt'
		temp_table = 'temperatures.php'
		temp_table_template = 'es_example_table.php'
...
usecols=list(range(len(templabels_array[0,:]))), invalid_raise = False )
		...
		table_output = open('table_of_temperatures.php', 'w')
		for i in range(29):
			table_output.write(template.readline())
		for i in range(len(templabels_array[0,:])):
			fieldlist=[L[i],descriptions[i],tempstructure[i,-1],tempstructure[i,-1],tempstructure[i,-1],tempstructure[i,-1],L[i][1]]
			#table_output.write('  <tr>\n'+
#''.join('    <td class=\"tg-yw41\">{0}</td>\n'.format(field) for field in fieldlist)+
#'  </tr>\n')
			table_output.write("""  <tr>
    <td class=\"tg-yw41\">{0}</td>
    <td class=\"tg-yw41\">{1}</td>
    <td class=\"tg-yw41\">{2:6.2f}</td>
    <td class=\"tg-yw41\">{3:6.2f}</td>
    <td class=\"tg-yw41\">{4:6.2f}</td>
    <td class=\"tg-yw41\">{5:6.2f}</td>
    <td class=\"tg-yw41\">{6}</td>
  </tr>
""".format(*fieldlist))
		...
		pfmts = ['r', 'k']
		zone1 = [20,8,10,5]	#a5, a6, b6, b8
		zone2 = [2,7]		#a2, b5
		zone4 = [14,19,13,18]	#b1, b2, b10, b11
		zone5 = [11,4,3,12,9,6]	#a3, a4, b9, b13, b7
		zone6 = [1,0,16,15]	#a0, a1, b3
